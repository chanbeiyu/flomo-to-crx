...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    ...
    if (cmd == "content.injectJs") {
        ...
        sendResponse({ message: "inject js end" });
    } else if (cmd == "content.load.flomo") {
        const result = loadFlomoMemos();
        sendResponse(result);
    }
    return true;
});
...
// 获取所有 flomo 数据
function loadFlomoMemos(callback) {
    let code;
    let memos;
    let message;
